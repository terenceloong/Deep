function imu = IMU_ADI_read(filepath)
...
n = length(data);
imu = zeros(n,7); %IMU数据,[t, wx,wy,wz, fx,fy,fz], deg/s, g
imu(:,2:7) = data(:,9:14);
t0 = UTC2GPS(data(1,2:7), 0); %第一个点的时间,[week,second],时区是0
ts = t0(2);
imu(1,1) = ts + data(1,8)/10000;
for k=2:n
    if data(k,7)~=data(k-1,7) %与前面一个点的秒数不一样,秒数加1
        ts = ts+1;
    end
    imu(k,1) = ts + data(k,8)/10000;
end

%% 检查时间是否正确
time_diff = diff(imu(:,1));
