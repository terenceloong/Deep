...
% 整理数据
% almanac = [ID, health, week, toe, af0, af1, sqa, e, M0, omega, Omega0, Omega_dot, i0];
almanac = zeros(32,13);
almanac(:,1)  = 1:32;        %ID
almanac(:,2)  = temp(:,1);   %health
almanac(:,3)  = temp(:,12);  %week
almanac(:,4)  = temp(:,3);   %toe,s
almanac(:,5)  = temp(:,10);  %af0,s
almanac(:,6)  = temp(:,11);  %af1,s/s
