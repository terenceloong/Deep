%% 验证静止状态下以速度角速度为量测的惯导滤波
% 状态量:姿态失准角,速度误差,陀螺仪零偏,加速度计零偏
% 量测量:速度误差,角速度
% 认为静止时陀螺仪输出为带噪声的陀螺仪零偏量测
...
H(1:3,4:6) = eye(3);
H(4:6,7:9) = eye(3);
R = diag([[1,1,1]*0.01, [1,1,1]/180*pi*sigma_gyro])^2;
...
%         X = X - P*Y'/(Y*P*Y')*(Y*X-d);
        %++++航向角修正量约束为0,水平加计零偏估计值约束为0
%         Y = zeros(3,12);
%         Y(1,1) = Cnb(1,1)*Cnb(1,3);
%         Y(1,2) = Cnb(1,2)*Cnb(1,3);
%         Y(1,3) = -(Cnb(1,1)^2+Cnb(1,2)^2);
%         Y(2,10) = 1;
%         Y(3,11) = 1;
%         X = X - P*Y'/(Y*P*Y')*Y*X;
        %++++航向角约束为不变,水平加计零偏估计值约束为0
%         Y = zeros(3,12);
%         Y(1,1) = C11*Cnb(1,3);
%         Y(1,2) = C12*Cnb(1,3);
%         Y(1,3) = -(C11*Cnb(1,1)+C12*Cnb(1,2));
%         d1 = C11*Cnb(1,2) - C12*Cnb(1,1);
%         Y(2,10) = 1;
%         Y(3,11) = 1;
%         d = [d1; 0; 0];
%         X = X - P*Y'/(Y*P*Y')*(Y*X-d);
...
plot_acc_esti(t, output.bias(:,4:6), cond.bias_acc/1000, output.P(:,10:12))